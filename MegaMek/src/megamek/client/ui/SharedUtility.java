...
public class SharedUtility {
    ...
    public static String doPSRCheck(MovePath md, Client client) {
        ...
        for (final Enumeration<MoveStep> i = md.getSteps(); i.hasMoreElements();) {
            ...
            if (!i.hasMoreElements() && !firstStep) {
                if ((entity instanceof Mech) || (entity instanceof VTOL)) {
                    rollTarget = mechOperations(client, nagReport, entity, rollTarget, step);
                } else if (entity instanceof Tank) {
                    rollTarget = tankOperations(nagReport, entity, rollTarget, step);
                }
            }
        ...
        if (entity instanceof Aero) {
            aeroOperations(md, client, nagReport, entity, overallMoveType);
        }

        return nagReport.toString();
    }
	private static PilotingRollData mechOperations(Client client, StringBuffer nagReport, final Entity entity,
			PilotingRollData rollTarget, final MoveStep step) {
	...
	private static PilotingRollData tankOperations(StringBuffer nagReport, final Entity entity,
			PilotingRollData rollTarget, final MoveStep step) {
		if ((step.getMovementType() == IEntityMovementType.MOVE_WALK) || (step.getMovementType() == IEntityMovementType.MOVE_VTOL_WALK) || (step.getMovementType() == IEntityMovementType.MOVE_RUN) || (step.getMovementType() == IEntityMovementType.MOVE_VTOL_RUN)) {
	...
	private static void aeroOperations(MovePath md, Client client, StringBuffer nagReport, final Entity entity,
			int overallMoveType) {
		PilotingRollData rollTarget;
